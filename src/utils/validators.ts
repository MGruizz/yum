...
export const RegisterValidate = Yup.object().shape({
    email: Yup.string().email("Ingrese un email con un formato válido (ejemplo: example@email.com)").trim().required("El email es requerido."),
    password: Yup.string().trim().min(6, 'La contraseña debe tener al menos 6 caracteres').required("El password es requerido."),
